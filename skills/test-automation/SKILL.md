...
> 若 `platforms.{ios,android}.repo` 已設定，可從遠端 repo 抓取 pattern；否則用本地專案。
...
| TC 分類 | 測試類型 | iOS 框架 | Android 框架 |
|---------|---------|---------|-------------|
| 冒煙測試（4 階段） | UI Test | XCUITest | Espresso |
| 功能測試 | Unit + UI | Swift Testing + XCUITest | JUnit + Espresso |
| 異常/邊界測試 | Unit Test | Swift Testing | JUnit + Mockk |
| 端對端測試 | UI Test | XCUITest | Espresso |
| 效能測試 | Performance Test | XCTest.measure | Benchmark |
| 並發安全測試 | Unit Test (TSAN) | Swift Testing + Actor | JUnit + Coroutine |
| API 驗證測試 | Unit Test | Swift Testing + Stub | JUnit + Mockk |

**腳本生成規則：**

iOS pattern 詳見 [`ios-patterns.md`](./ios-patterns.md)，Android pattern 詳見 [`android-patterns.md`](./android-patterns.md)。
